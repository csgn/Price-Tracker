...
import { Accordion, Badge, Table, Breadcrumb } from 'react-bootstrap';
...
export default function ProductDetail(props) {
  ...
  return (
    <div>
      <div>
        <Breadcrumb>
          <Breadcrumb.Item href={category.url}>{category.name}</Breadcrumb.Item>
          {subcategory.map((el) => {
                ...
                {sortedSuppliersOfPrice.map((el) => {
                  return (
                    <tr key={el.supplierid}>
                      <td>
                        <a className="h6 text-decoration-none" href={el.url}>
                          {el.name}
                        </a>
                        <Badge
